...
async function readResponse(response, fallback) {
  ...
  if (!response.ok || data.success === false)
    throw new Error(data.errorMessage || data.message || fallback)
  return data
}
...
export default function KnowledgePage() {
  ...
  const disableDocument = async (document) => {
    if (
      !window.confirm(`确定禁用“${document.title || '未命名文档'}”吗？禁用后 Agent 将不再检索它。`)
    )
      return
    ...
    try {
      const response = await fetch(
        `/api/knowledge/documents/${encodeURIComponent(document.documentId)}/disable`,
        { method: 'POST' }
      )
  ...
  const deleteDocument = async (document) => {
    if (!window.confirm(`确定永久删除“${document.title || '未命名文档'}”吗？此操作不可撤销。`))
      return
    setBusyId(document.documentId)
    setNotice(null)
    try {
      const response = await fetch(
        `/api/knowledge/documents/${encodeURIComponent(document.documentId)}`,
        { method: 'DELETE' }
      )
  ...
  const enableDocument = async (document) => {
    if (
      !window.confirm(
        `确定重新启用“${document.title || '未命名文档'}”吗？系统会重新生成它的 RAG 向量。`
      )
    )
      return
    setBusyId(document.documentId)
    ...
    try {
      const response = await fetch(
        `/api/knowledge/documents/${encodeURIComponent(document.documentId)}/enable`,
        { method: 'POST' }
      )
    ...
    <div className="knowledge-page">
      ...
      <header className="knowledge-page-head">
        <div>
          ...
          <p>管理会被股票 Agent 检索的研究资料和业务知识。</p>
        </div>
        <Link className="button subtle back-chat" to="/">
          <ArrowRight size={15} />
          返回问答
        </Link>
      </header>
      {notice ? (
        <div
          className={`knowledge-notice ${notice.type}`}
          role={notice.type === 'error' ? 'alert' : 'status'}
          aria-live="polite"
        >
          {notice.type === 'error' ? <TriangleAlert size={16} /> : <CheckCircle2 size={16} />}
          {notice.message}
        </div>
      ) : null}
      <main className="knowledge-layout">
        <section className="panel knowledge-form-panel">
          <KnowledgeForm busy={formBusy} onSubmit={addDocument} />
        </section>
        <section className="panel knowledge-list-panel">
          <div className="knowledge-list-head">
            <div>
              ...
              <p>
                {documents.length} 篇文档 · {enabledCount} 篇正在参与 RAG
              </p>
            </div>
            <button
          ...
          <KnowledgeList
            ...
            loading={loading}
            busyId={busyId}
            onDisable={disableDocument}
            onEnable={enableDocument}
