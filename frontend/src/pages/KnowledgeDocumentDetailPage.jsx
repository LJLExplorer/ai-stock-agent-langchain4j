...
export default function KnowledgeDocumentDetailPage() {
  ...
  const linkedDocument =
    location.state?.document?.documentId === documentId ? location.state.document : null
  ...
  useEffect(() => {
    let active = true
    // 路由快照仅用于首屏，重新进入或刷新详情时仍以服务端数据为准。
    setDocument(linkedDocument)
    setState(linkedDocument ? 'ready' : 'loading')
    setMessage('')
    ...
    <div className="knowledge-page knowledge-detail-page">
      <header className="topbar knowledge-topbar">
        ...
        <nav className="route-nav" aria-label="主导航">
          <NavLink to="/" end>
            <ArrowRight className="route-back-icon" size={15} />
            问答
          </NavLink>
      ...
      <main className="knowledge-detail-shell" tabIndex="-1">
        <Link className="detail-back" to="/knowledge">
          <ArrowLeft size={15} />
          返回知识库
        </Link>
        {state === 'loading' ? (
          <div className="knowledge-detail-state">
            <LoaderCircle className="spin" size={22} />
            <p>正在加载知识文档…</p>
          </div>
        ) : null}
        {state === 'not-found' ? (
          <div className="knowledge-detail-state" role="alert">
            <TriangleAlert size={24} />
            <h1>未找到知识文档</h1>
            <p>{message || '该文档可能已被删除。'}</p>
            <Link className="button subtle" to="/knowledge">
              返回知识库
            </Link>
          </div>
        ) : null}
        ...
        {state === 'ready' ? (
          <article className="panel knowledge-detail">
            <header className="knowledge-detail-head">
              <div>
                <div className="eyebrow">KNOWLEDGE DOCUMENT</div>
                <h1>
                  <FileText size={23} />
                  {pageTitle}
                </h1>
                <div className="knowledge-meta">
                  <span>{document.documentType || 'MANUAL'}</span>
                  <span>
                    <Database size={12} />
                    {document.chunkCount || 0} 个分块
                  </span>
                  <span>
                    <CalendarClock size={12} />
                    更新于 {formatTime(document.updateTime || document.createTime)}
                  </span>
                </div>
              </div>
              <span
                className={`status-badge ${document.enabled !== false ? 'active' : 'inactive'}`}
              >
                {document.enabled !== false ? '已启用' : '已禁用'}
              </span>
            </header>
            {document.tags?.length ? (
              <div className="knowledge-tags detail-tags">
                {document.tags.map((tag, index) => (
                  <span key={`${tag}-${index}`}>#{tag}</span>
                ))}
              </div>
            ) : null}
            <dl className="knowledge-detail-info">
              <div>
                <dt>创建时间</dt>
                <dd>{formatTime(document.createTime)}</dd>
              </div>
              <div>
                <dt>更新时间</dt>
                <dd>{formatTime(document.updateTime)}</dd>
              </div>
            </dl>
            <section
            ...
            >
              <h2 id="knowledge-content-title">正文内容</h2>
              <pre>{document.rawContent || '该文档没有可显示的正文内容。'}</pre>
            </section>
          </article>
        ) : null}
      </main>
