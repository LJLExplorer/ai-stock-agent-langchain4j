...
export default function KnowledgeList({ documents, loading, busyId, onDisable, onDelete }) {
  ...
  return <div className="knowledge-list">{documents.map((document) => {
    ...
    return <article className={`knowledge-card ${enabled ? '' : 'disabled'}`} key={document.documentId}>
      <div className="knowledge-card-main"><span className="knowledge-card-icon"><FileText size={17} /></span><div className="knowledge-card-copy"><div className="knowledge-card-title"><h3>{document.title || '未命名文档'}</h3><span className={`status-badge ${enabled ? 'active' : 'inactive'}`}>{enabled ? '已启用' : '已禁用'}</span></div><div className="knowledge-meta"><span>{document.documentType || 'MANUAL'}</span><span><Database size={12} />{document.chunkCount || 0} 个分块</span><span><CalendarClock size={12} />{formatTime(document.updateTime || document.createTime)}</span></div>{document.tags?.length ? <div className="knowledge-tags">{document.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div> : null}</div></div>
      <div className="knowledge-card-actions">{enabled ? <button className="text-action warning" onClick={() => onDisable(document)} disabled={busy} title="禁用后不再参与 RAG 检索"><Ban size={14} />{busy ? '处理中…' : '禁用'}</button> : <span className="disabled-note">已停止检索</span>}<button className="text-action danger" onClick={() => onDelete(document)} disabled={busy}><Trash2 size={14} />删除</button></div>
    </article>
  })}</div>
}
