...
- 复查 trace `179c70ac-c6f5-466f-b050-227c0e445ca7`：工具仅耗时 228ms，固定六角色加 Judge 的 7 次模型调用却串行累加到 304701ms；最终在 13:28:34 正常完成，并非死锁。
- 深度审议改为按 EvidencePack 范围选择专家并行执行，技术请求跳过 FUNDAMENTAL/NEWS，仅并行运行 TECHNICAL、BULL、BEAR、RISK 后再执行一次 Judge；角色日志补充角色名与耗时。
- 角色提示明确禁止把未请求板块描述为缺失或损坏；进度改用后端实际 `roleCount`，同时显示并行活动角色和真实耗时，墙钟计时不参与百分比。
- EvidencePack 事实映射为可持久化来源，裸 evidenceId 在回答中显示为“证据：来源名”，可直接打开可信 HTTP(S) 原文或定位来源项。
- 角色上下文补充权威分析日期、数据截止日和实际证据范围，避免把 2026-09-04 相对于 2026-09-06 误写成“未来数据”；风险字段禁止输出角色乱码、上游质量等内部流水线问题。
